...
def configure(
  ...
  if adversarial:
      ...
      #discr_loss = tf.reduce_mean( (real_pred - 1)**2 + (fake_pred)**2) + 10*gradient_penalty
      ...
      discr_loss = tf.reduce_mean(fake_pred - real_pred) / 1 + 10*gradient_penalty + 5.e-5*discr_l2_loss
      gen_loss = -tf.reduce_mean(fake_pred) / 1 + 5.e-5*gen_l2_loss
