...
def crop_image_batch(images, crops):

    cropped_images = tf.map_fn(
        lambda inputs: crop_image(*inputs), 
        elems=[images, crops],
        dtype=tf.float32
        )
...
def multiscale_discr(
    inputs, 
    batch_size, 
    discr_size=64, 
    sizes=[cropsize//(2**i) for i in range(3)], 
    reuse=False
    ):

    b = batch_size

    multiscale_real_preds = []
    multiscale_fake_preds = []
    multiscale_real_layers = []
    multiscale_fake_layers = []
    for size in sizes:
        
        if size == cropsize:
            crops = inputs
        else:
            #Crop at grid locations
            crops = []
            for x in range(0, cropsize, size):
                for y in range(0, cropsize, size):
                    crop = inputs[:,x:x+size,y:y+size,:]
                    crops.append(crop)
            crops = tf.concat(crops, axis=0)

            ##Randomly crop
            #crops = tf.random_crop(
            #    inputs, 
            #    [inputs.get_shape().as_list()[0], size, size, inputs.get_shape().as_list()[-1]]
            #    )

        if size != discr_size:
            crops = tf.image.resize_bilinear(crops, [discr_size, discr_size], align_corners=True)

        pred, layer = discriminator(crops)

        real_preds = []
        fake_preds = []
        real_layers = []
        fake_layers = []
        for i in range((cropsize//size)**2):
            s = 2*i*b

            real_pred = pred[s:s+b]
            fake_pred = pred[s:s+b]

            real_layer = layer[s+b:s+2*b]
            fake_layer = layer[s+b:s+2*b]

            real_preds.append(real_pred)
            fake_preds.append(fake_pred)
            real_layers.append(real_layer)
            fake_layers.append(fake_layer)

        if size != cropsize:
            real_preds = tf.stack(real_preds, axis=1)
            fake_preds = tf.stack(fake_preds, axis=1)
            real_layers = tf.concat(real_layers, axis=1)
            fake_layers = tf.concat(fake_layers, axis=1)
        else:
            real_preds = tf.expand_dims(real_preds[0], axis=1)
            fake_preds = tf.expand_dims(fake_preds[0], axis=1)
            real_layers = real_layers[0]
            fake_layers = fake_layers[0]

        multiscale_real_preds.append(real_preds)
        multiscale_fake_preds.append(fake_preds)
        multiscale_real_layers.append(real_layers)
        multiscale_fake_layers.append(fake_layers)

    return (multiscale_real_preds, multiscale_fake_preds), (multiscale_real_layers, multiscale_fake_layers)
...
def configure(
  ...
  if adversarial:
      ...
      output_wavefunc = output #tf.concat([inputs, output], axis=-1)
      target_wavefunc = target_outputs #tf.concat([inputs, target_outputs], axis=-1)
      discr_batch = tf.concat([output_wavefunc, target_wavefunc], axis=0)
      (real_preds, fake_preds), (real_layers, fake_layers) = multiscale_discr( discr_batch, batch_size )

      discr_loss_for_discr = []
      discr_loss_for_gen = []
      layer_loss = []
      for real_pred, fake_pred, real_layer, fake_layer in zip(
          real_preds, fake_preds, real_layers, fake_layers):

          discr_pred_loss = tf.reduce_mean( (real_pred - 1)**2 + fake_pred**2, axis=1 )
          gen_pred_loss = tf.reduce_mean( (fake_pred - 1)**2, axis=1 )

          abs_diff = tf.reduce_mean( tf.abs(real_layer - fake_layer) )

          discr_loss_for_discr.append(discr_pred_loss)
          discr_loss_for_gen.append(gen_pred_loss)
          layer_loss.append(abs_diff)
      discr_loss_for_discr = tf.add_n(discr_loss_for_discr)
      discr_loss_for_gen = tf.add_n(discr_loss_for_gen)
      layer_loss = tf.add_n(layer_loss)
  
      gen_loss = discr_loss_for_gen + 10*layer_loss
      discr_loss = discr_loss_for_discr
      ...
      gen_loss = alrc_mse + 1.e-4*l2_loss
  ...
  #gen_optimizer = tf.contrib.estimator.clip_gradients_by_norm(gen_optimizer, )
  ...
  #output_loss = tf.reduce_mean( tf.abs(output - target_outputs) )

  output_loss = {
      "Loss": tf.reduce_mean( tf.abs(output - target_outputs) ),
      "MSE": mse,
      }

  return train_op, output_loss, output


def to_phase(x):

    x = np.arctan2(x[...,1].reshape(cropsize, cropsize), x[...,0].reshape(cropsize, cropsize))
...
def experiment(report_every_n=100):
  ...
  data_dir = "//Desktop-sa1evjv/f/wavefunctions_single_refined/wavefunctions/"
  batch_size = 24
  ...
  model_dir = f"//ads.warwick.ac.uk/shared/HCSS6/Shared305/Microscopy/Jeffrey-Ede/models/wavefunctions/{EXPER_NUM}/"
  ...
  with tf.Session(config=config) as sess, open(log_filepath, "a") as log_file:
    ...
    for iter in range(start_iter, train_iters):
      ...
      is_training = True#iter < 50_000# or not is_halfway
      ...
      feed_dict = {
          ...
          beta1_ph: np.float32(beta1),
          mode_ph: np.int32(0 if iter < 50 else 1)
      }
      
      if iter < 101:
        step_loss, [step_image, step_target_output, step_output] = sess.run([
            loss,
            [first_image, first_target_output, first_output]
            ],
            feed_dict=feed_dict)
          
        save_input_loc = model_dir+"final_input-"+str(iter)+".tif"
        save_truth_loc = model_dir+"final_truth-"+str(iter)+".tif"
        save_output_loc = model_dir+"final_output-"+str(iter)+".tif"
        
        Image.fromarray(step_image.reshape(cropsize, cropsize).astype(np.float32)).save( save_input_loc )
        Image.fromarray(to_phase(step_target_output).astype(np.float32)).save( save_truth_loc )
        Image.fromarray(to_phase(step_output).astype(np.float32)).save( save_output_loc )
      ...
      if iter in [0, 100, 500] or not iter % 25_000 or (0 <= iter < 10_000 and not iter % 1000) or iter == start_iter:
        ...
        Image.fromarray(step_image.reshape(cropsize, cropsize).astype(np.float32)).save( save_input_loc )
        Image.fromarray(step_target_output[...,0].reshape(cropsize, cropsize).astype(np.float32)).save( save_truth_loc )
        Image.fromarray(step_output[...,0].reshape(cropsize, cropsize).astype(np.float32)).save( save_output_loc )
      else:
...
if __name__ == "__main__":
    ...
    with open(mode_dir+"val_loss.txt", "w") as f:
