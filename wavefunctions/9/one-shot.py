...
def configure(
  ...
  if adversarial:
      ...
      #discr_l2_loss = tf.add_n([tf.nn.l2_loss(v) for v in tf.trainable_variables("discr")])
      ...
      discr_loss = tf.reduce_mean( (real_pred - 1)**2 + \
          (fake_pred)**2, axis=1 )# + 1.e-5*discr_l2_loss
      gen_loss = tf.reduce_mean( (fake_pred - 1)**2, axis=1 )# + 1.e-5*gen_l2_loss
      ...
      total_loss = gen_loss + discr_loss + 10*gradient_penalty# + 1.e-5*(gen_l2_loss+discr_l2_loss)
