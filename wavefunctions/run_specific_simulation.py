...
OUTPUT_DIR = os.path.join(PARENT_DIR, "run_specific_output")
if not os.path.exists(OUTPUT_DIR):
    os.makedirs(OUTPUT_DIR)

NUM_REPEATS = 1

CONFIG_DIR = os.path.join(PARENT_DIR, "run_specific_temp")
config_filepath = os.path.join(CONFIG_DIR, "current_config.json")
...
def random_config():
    ...
    config["microscope"]["voltage"] = 80#random.choice(voltages)
    config["microscope"]["aperture"] = 15#np.random.uniform(5, 30)
    config["microscope"]["delta"] = 10#np.random.uniform(0, 20)
    config["microscope"]["alpha"] = 1#np.random.uniform(0.1, 2)
...
def do_sim(cif_filepath):
    ...
    for repetition in range(NUM_REPEATS):  # Number of times to go through CIFs
        ...
        cell_depth = 100#np.random.uniform(50, 1000)
        cell_widths = 75#np.random.uniform(50, 100)
        cell_string = "%s,%s,%s" % (cell_widths, cell_widths, cell_depth)

        # randomise the zone axis (only up to 2)
        zone_h = 1#np.random.randint(0, 3)
        zone_k = 1#np.random.randint(0, 3)
        zone_l = 1#np.random.randint(0, 3)
        zone_string = "%s,%s,%s" % (zone_h, zone_k, zone_l)

        # random tilt perturbations (normal distribution)
        tilt_a = 0.#np.random.normal(scale=0.1)
        tilt_b = 0.#np.random.normal(scale=0.1)
        tilt_c = 0.#np.random.normal(scale=0.1)
...
if __name__ == "__main__":
    ...
    filename = "20.cif"
    cif_filepath = os.path.join(CIFS_DIR, filename)

    do_sim(cif_filepath)

    amplitude_filepath = r"Z:\Jeffrey-Ede\models\wavefunctions\run_specific_output\0\0\EW_amplitude.tif"
    phase_filepath = r"Z:\Jeffrey-Ede\models\wavefunctions\run_specific_output\0\0\EW_phase.tif"

    amplitude = imread(amplitude_filepath, mode="F")
    phase = imread(phase_filepath, mode="F")

    disp(amplitude)
    disp(remap_phase(phase))
