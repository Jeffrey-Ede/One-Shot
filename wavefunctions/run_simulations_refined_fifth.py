...
CIFS_DIR = r"\\ads.warwick.ac.uk\shared\HCSS6\Shared305\Microscopy\Jeffrey-Ede\crystal_structures\standardized_inorganic_no_H"
cif_filepaths = [os.path.join(CIFS_DIR, f) for f in os.listdir(CIFS_DIR)]

PARENT_DIR = r"\\ads.warwick.ac.uk\shared\HCSS6\Shared305\Microscopy\Jeffrey-Ede\models\wavefunctions"
default_json_filepath = os.path.join(PARENT_DIR, "default.json")
failed_file_filepath = os.path.join(PARENT_DIR, "failed_files.txt")

EXE_DIR = r"\\ads.warwick.ac.uk\shared\HCSS6\Shared305\Microscopy\Jeffrey-Ede\models\wavefunctions\clTEM_file_0.2.4"
exe_filepath = os.path.join(EXE_DIR, "clTEM_cmd.exe")

OUTPUT_DIR = os.path.join(PARENT_DIR, "output_refined_fifth")
if not os.path.exists(OUTPUT_DIR):
    os.makedirs(OUTPUT_DIR)

NUM_REPEATS = 1

CONFIG_DIR = os.path.join(PARENT_DIR, "temp_refined_fifth")
config_filepath = os.path.join(CONFIG_DIR, "current_config_refined_fifth.json")
...
def do_sim(cif_filepath):
    ...
    for out_path in out_paths:
        ...
        cell_depth = np.random.uniform(50, 237.5)
        cell_widths = np.random.uniform(50, 62.5)
        cell_string = "%s,%s,%s" % (cell_widths, cell_widths, cell_depth)

        # randomise the zone axis (only up to 2)
        zone_h = zone_k = zone_l = 0
        while zone_h + zone_k + zone_l == 0:
            zone_h = np.random.randint(0, 2)
            zone_k = np.random.randint(0, 2)
            zone_l = np.random.randint(0, 2)
        zone_string = "%s,%s,%s" % (zone_h, zone_k, zone_l)

        # random tilt perturbations (normal distribution)
        tilt_a = np.random.normal(scale=0.1/4)
        tilt_b = np.random.normal(scale=0.1/4)
        tilt_c = np.random.normal(scale=0.1/4)
