...
def configure(
  ...
  if adversarial:
      ...
      avg_pred_fake = tf.reduce_mean(fake_pred)
      avg_pred_real = tf.reduce_mean(real_pred)

      mu_pred_fake = tf.get_variable(
        auto_name("mu_pred_fake"), 
        initializer=tf.constant(0.5, dtype=tf.float32),
        trainable=False
        )
      
      tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, mu_pred_fake.assign(0.99*mu_pred_fake+0.01*avg_pred_fake))
      ...
      discr_optimizer = tf.train.AdamOptimizer(
          learning_rate=0.0002*tf.sigmoid(15*(mu_pred_fake-0.5)),
          beta1=0.5
          )
  ...
  return train_op, output_loss, phase_components


def to_phase(x):

    x = np.angle(x[...,0] + 1j*x[...,1])
