...
def experiment(report_every_n=100):
  ...
  batch_size = 23 #GPU appears to have lower memory here. Typically 24.
  ...
  start_iter = 460_886#365_896#299_137#278_393#206_644#5_372#0
  train_iters = 500_000
  val_iters = 1_000
  ...
  with tf.Session(config=config) as sess:
    ...
    for iter in range(start_iter, train_iters):
      ...
      if iter in [0, 100, 500] or not iter % 25_000 or (0 <= iter < 10_000 and not iter % 1000) or iter == start_iter:
        ...
        x = sess.run(output, feed_dict=feed_dict)
        print("time", (time.time()-start_time)/batch_size)
