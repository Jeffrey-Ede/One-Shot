...
def large_discriminator(inputs):
    ...
    #x = res_block(x, 32, s=2)
    x = res_block(x, 64, s=2)
    x = res_block(x, 128, s=2)
    x = res_block(x, 256, s=2)
    x = res_block(x, 512, s=2)

    for _ in range(1):
        x = res_block(x, 512, s=1)
    ...
    x = 0.25*tf.reduce_sum(x, axis=[1,2])
...
def configure(
  ...
  if adversarial:
      ...
      #grad = tf.gradients(x_hat_pred, x_hat)
      ...
      #discr_loss = tf.reduce_mean( (real_pred - 1)**2 + \
      #    (fake_pred)**2)
      #gen_loss = tf.reduce_mean( (tracking_pred - 1)**2)
      ...
      #discr_loss = tf.reduce_mean(real_pred - fake_pred) / 1000
      #gen_loss = tf.reduce_mean(tracking_pred) / 1000
...
def experiment(report_every_n=100):
  ...
  with tf.Session(config=config) as sess, open(log_filepath, "a") as log_file:
    ...
    for iter in range(start_iter, train_iters):
      ...
      beta1 = 0.#0.5 if iter < 200_000 else 0.9
