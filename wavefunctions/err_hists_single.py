...
model_dir = "Z:/Jeffrey-Ede/models/wavefunctions/24/"

labels_sets = [["50000 Iter: Unseen V", 
                "50000 Iter: Unseen V, H", 
                "200000 Iter: Unseen V", 
                "200000 Iter: Unseen V, H",
                "200000 Iter: Unseen V, H, M"
                ]]
sets = [[
    "Z:/Jeffrey-Ede/models/wavefunctions/22/train_err.npy", 
    "Z:/Jeffrey-Ede/models/wavefunctions/22/val_err.npy", 
...
for i, (data_nums, labels) in enumerate(zip(sets, labels_sets)):
    ...
    plt.legend(loc='upper right', frameon=True, fontsize=8, facecolor='white', framealpha=1, edgecolor="white", bbox_to_anchor=(.98,.95))

    plt.axvline(x=0.75, color='black', linestyle='--', linewidth=0.6)

    save_loc =  model_dir + "hist_single.png"
