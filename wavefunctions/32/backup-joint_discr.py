...
def configure(
  ...
  if adversarial:
      ...
      r = tf.random_uniform(shape=[batch_size]) < 0.5
      c1 = tf.where(
          r,
          phase_components,
          target_outputs
      ...
      if use_gradient_penalty:
          ...
          discr_batch = inputs*tf.concat([c1, c2], axis=-1) #Wavefunction representation
      ...
      if use_gradient_penalty or standard_wass:
          ...
          discr_loss = tf.reduce_mean( (preds - labels)**2 )
          gen_loss = tf.reduce_mean( (preds - anti_labels)**2 )
