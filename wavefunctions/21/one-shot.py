...
def configure(
  ...
  if adversarial:
      ...
      output = tf.concat([inputs, (1+output)/amp(output)], axis=-1)
      target_outputs = tf.concat([inputs, (1+target_outputs)/amp(target_outputs)], axis=-1)
...
def experiment(report_every_n=100):
  ...
  start_iter = 0#38_459#49_801#38_459#0
  train_iters = 500_000
  val_iters = 1_000
