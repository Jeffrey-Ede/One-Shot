...
def conv(
    ...
    if is_batch_norm and use_batch_norm:
        x = tf.contrib.layers.batch_norm(x, decay=batch_norm_decay, is_training=is_training)
    if is_batch_norm and use_instance_norm:
...
def generator(inputs, num_outputs, is_training, is_depthwise_sep=False):
    ...
    #Residual blocks
    for _ in range(8): #Number of blocks
        x = residual_block(
            x, 
            skip=3,
            is_training=is_training
    ...
    for i in range(3, -1, -1):
        ...
        x = tf.image.resize_bilinear(x, [2*s for s in x.get_shape().as_list()[1:3]], align_corners=True)
...
def discriminator(inputs, return_layer_num=-1):
  ...
  #Fully connected
  x = spectral_norm_dense(x, 1)

  x = x[:, 0] #To avoid adjusting rest of code
...
def configure(
  ...
  if adversarial:
      ...
      xhat = output + tf.random_uniform(output.get_shape().as_list())*(target_outputs-output)

      discr_batch = tf.concat([output, target_outputs, xhat], axis=0)

      with tf.variable_scope("discr"):
        (real_preds, fake_preds, xhat_preds) = multiscale_discr( discr_batch, batch_size )


      discr_loss_for_discr = []
      discr_loss_for_gen = []
      for real_pred, fake_pred, x_hat_pred in zip(real_preds, fake_preds, xhat_preds):

          grad = tf.gradients(x_hat_pred, [xhat])[0]
          grad_norm2 = tf.sqrt(1e-8 + tf.reduce_sum(tf.square(grad), axis=[1,2,3]))
          gradient_penalty = tf.reduce_mean( (grad_norm2 - 1.)**2 )

          discr_pred_loss = tf.reduce_mean(fake_pred - real_pred) / 1 + 10*gradient_penalty
          gen_pred_loss = -tf.reduce_mean(fake_pred) / 1
      ...
      gen_loss = tf.reduce_mean(discr_loss_for_gen)
      discr_loss = tf.reduce_mean(discr_loss_for_discr)
      ...
      gen_loss = alrc_mse + 1.e-5*l2_loss
  ...
  with tf.control_dependencies(update_ops):
    if adversarial:
        ...
        discr_train_op = discr_optimizer.minimize(discr_loss, var_list=tf.trainable_variables("discr"))
        train_op = [gen_train_op, discr_train_op]
    else:
        train_op = gen_optimizer.minimize(gen_loss)
    
  output_loss = {
      "Loss": tf.reduce_mean( tf.abs((output - target_outputs)/inputs) ),
      "pred_real": tf.add_n([tf.reduce_mean(x) for x in real_preds])/3,
      "pred_fake": tf.add_n([tf.reduce_mean(x) for x in fake_preds])/3
      }
...
def experiment(report_every_n=100):
  ...
  with tf.Session(config=config) as sess, open(log_filepath, "a") as log_file:
    ...
    for iter in range(start_iter, train_iters):
      ...
      is_training = iter < 1_000 #not is_halfway
      ...
      if iter in [0, 100, 500] or not iter % 25_000 or (0 <= iter < 10_000 and not iter % 1000) or iter == start_iter:
        ...
        Image.fromarray(step_image.astype(np.float32)).save( save_input_loc )
        Image.fromarray((step_target_output[...,0]/step_image).astype(np.float32)).save( save_truth_loc )
        Image.fromarray((step_output[...,0]/step_image).astype(np.float32)).save( save_output_loc )
      else:
        #for step_train_op in [train_op[1], train_op[1], train_op[1], train_op[1], train_op]:
