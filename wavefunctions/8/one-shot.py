...
def configure(
  ...
  if adversarial:
      ...
      output_wavefunc = tf.concat([inputs, output], axis=-1)
      target_wavefunc = tf.concat([inputs, target_outputs], axis=-1)
...
def experiment(report_every_n=100):
  ...
  with tf.Session(config=config) as sess, open(log_filepath, "a") as log_file:
    ...
    for iter in range(start_iter, train_iters):
      ...
      is_training = iter < 50_000 #not is_halfway
      ...
      if iter in [train_iters//2-1, train_iters-1]:
          saver.save(sess, save_path=model_dir+"noteable_ckpt/model", global_step=iter)
          time0 = time.time()
